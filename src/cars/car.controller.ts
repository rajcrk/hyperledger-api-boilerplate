...
export class CarController {
     ...
     */
    @Get(':id')
    @ApiOperation({ title: 'Get food by id' })
    @ApiOAuth2Auth(['read'])
    @ApiResponse({
        status: 200,
        description: 'Returns a Food object',
        type: CarDto,
    })
    @ApiResponse({
        status: 404,
    ...
    getFoodById(@Param('id') id: string): Promise<CarDto> {
        return this.carService.getFoodById(id);
    }
     ...
     */
    @Get(':id')
    @ApiOperation({ title: 'Get a food by id' })
    @ApiOAuth2Auth(['read'])
    @ApiResponse({
        status: 200,
